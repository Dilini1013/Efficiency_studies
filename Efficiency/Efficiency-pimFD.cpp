...
//////put this first, you may need a different function


Double_t  f_theta_max_pim(Double_t *x, Double_t *par);
...
void Efficiency(){
    ...
    TFile *AnaFile = new TFile("efficiency_datatest.root", "RECREATE", "Test");
    ...
    auto* h_vertexZ=new TH1D("VertexZ","Vz disribution for CD particles",200,-30,30);
    ...
    auto*  h_Track_CVT_Chi2N = new TH1D("h_Track_CVT_chi2N","Chi2/NDF CVT; ChiSq/NDF_CVT; Events  ", 100.,0.0,100.0);
    ...
    auto* h_chi2pid_proton_CD=new TH1D("h_chi2pid_proton_CD","chi2pid_proton in CD;Events ",100,-7.0,7.0);
    auto* h_chi2pid_piplus_CD=new TH1D("h_chi2pid_piplus_CD","chi2pid_piplus in CD;Events ",100,-7.0,7.0);
    auto* h_chi2pid_piminus_CD=new TH1D("h_chi2pid_piminus_CD","chi2pid_piminus in CD;Events ",100,-7.0,7.0);
    ...
    auto* h_betapmag_CTOF_positive = new TH2D(" h_betapmag_CTOF_positive "," #beta vs p for positive in CTOF;p(GeV);#beta ",200, 0.0, 2.0,200,0.2,1.6);
    auto* h_betapmag_CTOF_negative = new TH2D(" h_betapmag_CTOF_negative "," #beta vs p for negative in CTOF;p(GeV);#beta ",200, 0.0, 2.0,200,0.2,1.6);
    ...
    auto* h_betapmag_CTOF_proton = new TH2D(" h_betapmag_CTOF_proton "," #beta vs p for proton in CTOF;p(GeV) ;#beta ",200, 0.0, 2.0,200,0.2,1.6);
    auto* h_betapmag_CTOF_piplus = new TH2D(" h_betapmag_CTOF_piplus "," #beta vs p for piplus in CTOF;p(GeV) ;#beta ",200, 0.0, 2.0,200,0.2,1.6);
    auto* h_betapmag_CTOF_piminus = new TH2D(" h_betapmag_CTOF_piminus "," #beta vs p for piminus in CTOF;p(GeV) ;#beta ",200, 0.0, 2.0,200,0.2,1.6);
    ...
    auto* h_piplus_P= new TH1D("h_piplus_P","Momentum; P(GeV);Events",200,0.,10.);
    auto* h_kaonplus_P= new TH1D("h_kaonplus_P","Momentum; P(GeV);Events",200,0.,10.);
    auto* h_piminus_P= new TH1D("h_piminus_P","Momentum; P(GeV);Events",200,0.,10.);
    auto* h_kaonminus_P= new TH1D("h_kaonminus_P","Momentum; P(GeV);Events",200,0.,10.);
    
    auto* h_efficiency_eppipX= new TH1D("h_efficiency_eppipX","H(e, e' p pi+)X MM2;M_{X}^2(e p \pi+) GeV^2; Events ",
                                        300.,-0.5,0.5);
    auto* h4= new TH1D("h4","H(e, e' p pi+)X MM2;M_{X}^2(e p \pi+) GeV^2; Events ",
                       300.,-0.5,0.5);
    auto  *h3fold= new TH1D("h3fold","H(e, e' p pi+)X MM2;M_{X}^2(e p \pi+) GeV^2; Events ",
                            300.,-0.5,0.5);
    h3fold->Sumw2();
    auto* h_efficiency_eppip_pimX= new TH1D(" h_efficiency_eppip_pimX","H(e, e' p pi+)X MM2;M_{X}^2(e p \pi+) GeV^2; Events ",
                                            200.,-0.5,0.5);
    auto* h_efficiency_eppip_fake= new TH1D("h_efficiency_eppip_fake","H(e, e' p pi+)X MM2;M_{X}^2(e p \pi+) GeV^2; Events ",
                                            200.,-0.5,0.5);
    
    auto* h_efficiency_eppippimX= new TH1D("h_efficiency_eppippimX","H(e, e' p pi+ pi-)X MM2;M^2(e p \pi+ \pi- X) GeV^2; Events ",
                                           200.,-0.5,0.5);
    
    auto* h_momP3fold=new TH1F("h_momP3fold","H(e, e' p pi+)X Piminus_mom", 200.,0.0,5.0);
    h_momP3fold->GetXaxis()->SetTitle(" P (GeV)");
    h_momP3fold->Sumw2();
    auto* h_momP4fold=new TH1F("h_momP4fold","H(e, e' p pi+ pi-)X Piminus_mom", 200.,0.0,5.0);
    h_momP4fold->GetXaxis()->SetTitle(" P (GeV)");
    h_momP4fold->Sumw2();
    
    auto* h_3foldthetaVsp = new TH2D("h_3foldthetaVsp"," 3 fold theta vs P data;p(GeV);#theta  ",100, 0.0, 5.0,300,0.0,150);
    //  h_3foldthetaVsp->Sumw2();
    auto* h_3foldthetaVsphi = new TH2D("h_3foldthetaVsphi"," 3 fold theta vs Phi data;#phi;#theta ",240, -180.,180.,300,0.,150.);
    //h_3foldthetaVsphi->Sumw2();
    auto* h_4foldthetaVsp = new TH2D("h_4foldthetaVsp"," 4 fold theta vs P data;p(GeV);#theta  ",100, 0.0, 5.0,300,0.0,150);
    // h_4foldthetaVsp->Sumw2();
    auto* h_4foldthetaVsphi = new TH2D("h_4foldthetaVsphi"," 4 fold theta vs Phi;#phi;#theta ",300,-180., 180.,300,0.,150.);
    
    
    
    auto* h_4foldtheta = new TH1D("h_4foldtheta"," 4 fold theta for pi-missing;#theta  ",200,0.0,150);
    auto* h_4foldphi = new TH1D("h_4foldphi"," 4 fold Phi for pi- missing;#phi ",300,-180., 180.);
    
    auto h_sinthetaPvsPhi=new TH2D ("h_sinthetaPvsPhi" , "sin(theta)/p vs phi;#phi;sin(#theta)/p", 300,-180.,180,300,0.0,1.0);
    
    //Efficiency Studies
    auto* h_Missing_total_mass=new TH1F("h_Missing_total_mass"," M_X^2 ", 300,-0.05,0.05);
    h_Missing_total_mass->Sumw2();
    auto* h_Missing_total_energy=new TH1F("h_Missing_total_energy"," E_X ", 300,-1.0,1.0);
    h_Missing_total_energy->Sumw2();
    auto* h1=new TH1F("h1","M^2(pi- +X) missing pi- mass^2", 300,-0.5,0.5);
    h1->Sumw2();
    ...
    auto* h_4foldthetaVsP_piminus = new TH2D("h_4foldthetaVsP_piminus"," (piminusFD)theta(pi-)  vs p(pi-);p(GeV);#theta  ",100, 0.0, 5.0,300,0.0,60);
    ...
    auto * h_pim_eff = new TH2F("h_pim_eff","#pi^{-} Efficiency i(#theta) vs p; p (GeV); i_{#theta} ",n_p,0.0,pmax_h, i_theta_bins,0.0,(double) i_theta_bins);
    auto *h2_pim_3fold_exc = new TH2F("h2_pim_3fold_exc","H(e,e^{'}p #pi^{+})X  exclusive events(3fold)  i(#theta) vs p; p (GeV); i(#theta)", n_p,0.0,pmax_h, i_theta_bins,0.0,(double) i_theta_bins);
    auto *h2_pim_4fold_bgnd = new TH2F("h2_pim_4fold_bgnd","H(e,e^{'}p #pi^{+}#pi^{-})X events background(4fold) i(#theta) vs p; p (GeV); #theta (deg)",n_p,0.0,pmax_h, i_theta_bins,0.0,(double) i_theta_bins);
    auto *h2_pim_4fold_exc = new TH2F("h2_pim_4fold_exc","H(e,e^{'}p #pi^{+}#pi^{-})X inclusive events(4fold)  i(#theta) vs p; p (GeV); #theta (deg)",n_p,0.0,pmax_h, i_theta_bins,0.0,(double) i_theta_bins);
    auto *h2_pim_3fold_bgnd = new TH2F("h2_pim_3fold_bgnd","H(e,e^{'}p #pi^{+}) X events bgnd(3fold)  i(#theta) vs p; p (GeV); #theta (deg)",n_p,0.0,pmax_h, i_theta_bins,0.0,(double) i_theta_bins);
    
    
    TF1 * theta_min_pip=nullptr;
    TF1 * theta_min_pim=nullptr;
    TF1 * theta_max_pim=nullptr;
    
    theta_min_pip= new TF1("theta_min_pip", "[0] + [1]/x", 0.5,5.0 );
    theta_min_pip->SetParameter(0,5);
    theta_min_pip->SetParameter(1,5);
    
    theta_min_pim= new TF1("theta_min_pim", "[0] + [1]/x", 0.5,5.0 );
    theta_min_pim->SetParameter(0,10);
    theta_min_pim->SetParameter(1,10);
    
    theta_max_pim= new TF1("theta_max_pim",f_theta_max_pim,0.5,5.0,4);
    theta_max_pim->SetParameter(0,40.0);
    theta_max_pim->SetParameter(1,1.5);
    theta_max_pim->SetParameter(2,45.0);
    theta_max_pim->SetParameter(3,0.5);
   ...
   fake.Add("/cache/clas12/rg-a/production/recon/fall2018/torus-1/pass1/v0/dst/train/skim4/skim4_00503*.hipo");
    ...
    //         fake.Add("/cache/clas12/rg-a/production/recon/fall2018/torus-1/pass1/v0/dst/train/skim4/skim4_005120.hipo");
   ...
   //   fake.Add("/lustre19/expphy/volatile/clas12/dilinib/SIDIS/MC_testing/Background_merge/skim_orlando_new45nA_job_*.hipo");
    ...
    //   fake.Add("/volatile/clas12/dilinib/SIDIS/Orlando_files/45nA_job*.hipo");
    ...
    TLorentzVector Lambda,M2K,M122_calctrigger,M122_calchighE,electron,photon,Miss_pim, Miss_pip,electronhighE,photonBrems, M122_calctriggerwithBremsphoton, Miss_total,pionHighE,miss_doublepion,proton_piplus,proton_piminus,pion_invariant;
    ...
    TLorentzVector efficiency_eppip,efficiency_eppippim, efficiency_eppip_pimX, Fake_pim;
    ...
    double Mn=db->GetParticle(2112)->Mass();
    double Mk=db->GetParticle(321)->Mass();
    
    double Mphoton=db->GetParticle(22)->Mass();    ///    ????????????????????????????????????
    ...
    double Mpi=db->GetParticle(211)->Mass(); //0.13957;
    double mPi2 = Mpi*Mpi;
    //  double sigma_0=0.01;
    //   double sigma_theta=0.001;
    ...
    unsigned long int piplusmultCD=0;
    unsigned long int piminusmultCD=0;
    unsigned long int protonmultCD=0;
    int photonmult=0;
    ...
    double Exc_pi_window=0.15;
    double MX2pim, MX2pip, MX2proton;
    double MX2pim_4fold;
    double itheta, dtheta, ppim, ppip;
    double itheta_4fold,ppim_4fold;
    double bgnd_low = 0.3, bgnd_hi = 0.5;
    int ntheta=3;
    ...
    double REC_Track_DC,Rec_Track_DC_Chi2,Rec_Track_DC_Chi2N,Track_CVT_Chi2N;
    double PCAL_lu, PCAL_lv, PCAL_lw, PCAL_sector;
    unsigned long int multiplex = 16;
    //  For each event, store a list of particle# pointers for valid protons, piplus, piminus
    //  This replaces the i_protons, i_piplus, i_piminus "multiplexing" which was flawed
    const int pid_multiplicity = 64;
    int n_protons[pid_multiplicity], n_piplus[pid_multiplicity], n_piminus[pid_multiplicity];
    ...
    for(Int_t i=0;i<files->GetEntries();i++){
        ...
        while(c12.next()==true){
            ...
            unsigned long int i_protons_CD=0;
            unsigned long int i_piplus_CD = 0;
            unsigned long int i_piminus_CD = 0;
            ...
            for(int i=0;i<(int)particles.size();i++){
                ...
                //Vz distribution for CD
                if(particles[i]->getRegion()==CD){
                    h_vertexZ->Fill(vz);
                }
                
                //CD beta vs mom plots for negative and positive charge partivcles
                
                if(charge > 0 && particles[i]->getRegion()==CD){
                    h_betapmag_CTOF_positive->Fill(pmag, beta);
                    if(pid==2212) h_betapmag_CTOF_proton->Fill(pmag, beta);
                    if(pid==211) h_betapmag_CTOF_piplus->Fill(pmag,beta);
                }
                if(charge <0 && particles[i]->getRegion()==CD){
                    h_betapmag_CTOF_negative->Fill(pmag, beta);
                    if(pid==-211)h_betapmag_CTOF_piminus->Fill(pmag, beta);
                }
                
                //CVT Tracking
                Track_CVT_Chi2N=particles[i]->trk(CVT)->getChi2N();
                h_Track_CVT_Chi2N->Fill(particles[i]->trk(CVT)->getChi2N());
                
                
                if(!db->GetParticle(pid)) continue;
                nsize++;
                
                //Applying momentum correction
                
                
                int esec=particles[i]->cal(PCAL)->getSector();
                int pisec=particles[i]->sci(FTOF1A)->getSector();
                
                double xx[] = {
                    0.0287513, 0.0130695, 0.0145024, -0.00228007, 0.00748802, 0.0143995,
                    0.00952351, 0.00394088, -0.0203049, 0.0286722, 0.0122367, 0.0107967,
                    0.00460442, 0.0075647, 0.0288222, 0.00753398, 0.00108634, 0.0100401,
                    0.0212596, 0.00106437, -0.004053, 0.00493538, 0.0119195, 0.0623409,
                    0.00880768, -0.00707046, 0.00376312, 0.019264, 0.0114119, 0.0220557,
                    0.00555625, 0.00964952, 0.0470234, 0.00977712, 0.00075287, 0.0064658,
                    0.0132933, 0.00783653, 0.01307, 0.00259644, -0.00127707, 0.0497935,
                    0.0099296, 8.85517e-05, 0.00551338, 0.0261954, 0.00850628, 0.00958545,
                    0.00233285, 0.00796244, 0.0641775, 0.00978549, 0.000148052, -0.0100278,
                };
                ...
                //    double fe = dpp(px,py,pz,esec,0) + 1;
                //    double fpip = dpp(px,py,pz,pisec,1) + 1;
                //    double fpim = dpp(px,py,pz,pisec,2) + 1;
                double fe=1;
                double fpip=1;
                double fpim=1;
                ...
                bool DC_fiducial_region1_chi2= DC_fiducial_cut_chi2(particles[i],region[0]);
                bool DC_fiducial_region2_chi2= DC_fiducial_cut_chi2(particles[i],region[1]);
                bool DC_fiducial_region3_chi2= DC_fiducial_cut_chi2(particles[i],region[2]);
                ...
                if(particles[0]->par()->getPid()==11 &&  status > -3000 && status < -2000 && !electtrig){
                                ...
                                if( minimal_PCAL){
                                    if(Z_vertex_cut){
                                        if((particles[i]->par()->getChi2Pid()) > -2.5 && (particles[i]->par()->getChi2Pid()) < 2.5 ){
                                            ...
                                            electrontrigger.SetPxPyPzE(fe*px,fe*py,fe*pz,sqrt(pow(fe*pmag,2) + pow(Me,2)));
                                            //   if(simulation)smear(&electrontrigger,1);
                                            pindex_electron=i;
                                            electtrig=true;
                                            electronmult++;
                                        }
                    ...
                    if(particles[i]->getRegion()==FD){
                        ...
                        //  THESE NUMBERS +/- 20 should be replaced with a variable   CEH
                        if((electronVz-protonVz)> -20 && (electronVz-protonVz)<20 ){
                            if((particles[i]->par()->getChi2Pid()) > -2.5 && (particles[i]->par()->getChi2Pid()) < 2.5 ){
                                
                                if(DC_fiducial_region1_chi2 && DC_fiducial_region2_chi2 && DC_fiducial_region3_chi2){
                                    proton.SetPxPyPzE(px, py, pz,sqrt(pow(pmag,2) + pow(Mp,2)));
                                    //   if(simulation)smear(&proton,1);
                                    h_proton_polar->Fill(proton.Theta()*180./TMath::Pi());
                                    protonmult++;
                                    if (protonmult>=pid_multiplicity){
                                        printf("excessive proton multiplicity = %d\n",protonmult);
                                        break;
                                    }
                                    n_protons[protonmult-1] = i;
                                    /*
                    ...
                    }/* else if(particles[i]->getRegion()==CD){
                      //Vertex differnce cut in CD
                      if((electronVz-piplusVz)> -20 && (electronVz-piplusVz)<20 ){
                      //Chi2Pid cut for CD
                      if((particles[i]->par()->getChi2Pid()) > -4.7 && (particles[i]->par()->getChi2Pid()) < 4.7 ){
                      h_chi2pid_proton_CD->Fill(particles[i]->par()->getChi2Pid());
                      h_dvz1->Fill(electronVz-protonVz);
                      proton.SetPxPyPzE(px, py, pz,sqrt(pow(pmag,2) + pow(Mp,2)));
                      //   if(simulation)smearCD(&proton,1);
                      protonCDPid=true;
                        ...
                        if((electronVz-piplusVz)> -20 && (electronVz-piplusVz)<20 ){
                            if((particles[i]->par()->getChi2Pid()) > -2.64 && (particles[i]->par()->getChi2Pid()) < 2.64 ){
                                ...
                                if(DC_fiducial_region1_chi2 && DC_fiducial_region2_chi2 && DC_fiducial_region3_chi2){
                                    ...
                                    piplus.SetPxPyPzE(px,py,pz,sqrt(pow(pmag,2) + pow(Mpi,2)));
                                    //   if(simulation)smear(&piplus,1);
                                    h_pip_polar->Fill(piplus.Theta()*180./TMath::Pi());
                                    piplusPid=true;
                                    // h_dvz2->Fill(electronVz-piplusVz);
                                    piplusmult++;
                                    if (piplusmult>=pid_multiplicity){
                                        printf("excessive pip multiplicity = %d\n",piplusmult);
                                        break;
                                    }
                                    n_piplus[piplusmult-1]= i;
                                }
                            }
                        }
                    }/*else if(particles[i]->getRegion()==CD){
                      piplusVz=particles[i]->par()->getVz();
                      h_chi2pid_piplus_CD->Fill(particles[i]->par()->getChi2Pid());
                      //vertex differnce cut in CD
                      if((electronVz-piplusVz)> -20 && (electronVz-piplusVz)<20 ){
                      //Chi2Pid cut for CD
                      if((particles[i]->par()->getChi2Pid()) > -4.0 && (particles[i]->par()->getChi2Pid()) < 4.0 ){
                      piplus.SetPxPyPzE(px,py,pz,sqrt(pow(pmag,2) + pow(Mpi,2)));
                      //  if(simulation)smearCD(&piplus,1);
                      // }
                      piplusPid=true;
                      // h_dvz2->Fill(electronVz-piplusVz);
                      piplusmult++;
                    ...
                    if(particles[i]->getRegion()==FD){
                        ...
                        if((electronVz-piminusVz)> -20 && (electronVz-piminusVz)<20 ){
                            // if(particles[i]->getRegion()==FD){
                            if(DC_fiducial_region1_chi2 && DC_fiducial_region2_chi2 && DC_fiducial_region3_chi2){
                                if((particles[i]->par()->getChi2Pid()) > -2.79 && (particles[i]->par()->getChi2Pid()) < 2.79 ){
                                    //DC Fiducial cuts valid
                                    piminus.SetPxPyPzE(fpim*px,fpim*py,fpim* pz,sqrt(pow(fpim*pmag,2) + pow(Mpi,2)));
                                    //  if(simulation)smear(&piminus,1);
                                    h_pim_polar->Fill(piminus.Theta()*180./TMath::Pi());
                                    piminusPid=true;
                                    piminusmult++;
                                    if (piminusmult>=pid_multiplicity){
                                        printf("excessive pim multiplicity = %d\n",piminusmult);
                                        break;
                                    }
                                    n_piminus[piminusmult-1]= i;
                                }
                            }
                        }
                    } /* else if (particles[i]->getRegion()==CD){
                       h_chi2pid_piminus_CD->Fill(particles[i]->par()->getChi2Pid());
                       piminusVz=particles[i]->par()->getVz();
                       h_dvz3->Fill(electronVz-piminusVz);
                       //vertex differnce cut in CD
                       if((electronVz-piminusVz)> -20 && (electronVz-piminusVz)<20 ) {
                       //Chi2Pid cut in CD
                       if((particles[i]->par()->getChi2Pid()) > -4.1 && (particles[i]->par()->getChi2Pid()) < 4.1 ){
                       piminus.SetPxPyPzE(px,py, pz,sqrt(pow(fpim*pmag,2) + pow(Mpi,2)));
                       //     if(simulation)smearCD(&piminus,1);
                       h_pim_polar->Fill(piminus.Theta()*180./TMath::Pi());
                       piminusPid=true;
                       piminusmult++;
                       if (piminusmult>=pid_multiplicity){
                ...
                /*
                 if ( (piminusmult >0)&&(piplusmult>0)&&(electronmult>0)&&(protonmult>0) ) {
                 coinc_counter++;
                 }
                 */
            ...
            //    eventNumber++;;
            //            if(electronmult*protonmultCD*piplusmultCD*piminusmultCD==0) continue;
            
            Double_t x;
            
            
            
            if( (electronmult*protonmult*piplusmult==0))continue;
            
            //&&(electronmult*protonmult*piminusmult==0) )
            q4vec=beam-electrontrigger;
            // Delta=Q2-P122;
            // Delta=proton-target; // Move inside loop
            W4vec=beam+target-electrontrigger;
            WSq=W4vec.M2();
            QSq=-q4vec.M2();
            hQ2->Fill(QSq);
            ...
            if(QSq > 1.0 && WSq > 4.0){
                ...
                {
                    jndex =n_protons[iPart]; //(int)(i_protons%multiplex);
                    // i_protons = i_protons/multiplex;
                    
                    if(jndex==0) {
                        printf("proton error n_proton[%d] = %d \n", iPart, jndex);
                        printf("pindex_electron %i , electron3mom epx= %f,epy= %f,epz= %f \n", pindex_electron, electrontrigger.Px(),electrontrigger.Py(),electrontrigger.Pz());
                        continue;
                    }
                    ...
                    {
                        jndex = n_piplus[jPart]; // (int)(i_piplus%multiplex);
                        
                        
                        if(jndex==0) {
                            printf("piplus error n_piplus[%d] = %d \n", jPart, jndex);
                            continue;
                        }
                        //  printf("kndex %i, i_piplus %lu, mpiplus %i \n",kndex, i_piplus, piplusmult);
                        px = particles[jndex]->par()->getPx();
                        py = particles[jndex]->par()->getPy();
                        pz = particles[jndex]->par()->getPz();
                        pmag = particles[jndex]->par()->getP();
                        ...
                        double phiRot_3fold=efficiency_eppip.Phi()*TMath::RadToDeg() - (sin(efficiency_eppip.Theta())/efficiency_eppip.P())*70;
                        if(phiRot_3fold < -180) phiRot_3fold+=360;
                        if(((-20 <phiRot_3fold) && (phiRot_3fold < 20)) || ((40 <phiRot_3fold) && (phiRot_3fold < 80)) || ((100 <phiRot_3fold) && (phiRot_3fold < 140)) || ((-80 <phiRot_3fold) && (phiRot_3fold < -40)) || ((-140 <phiRot_3fold) && (phiRot_3fold < -100)) || ((160 <phiRot_3fold) || (phiRot_3fold < -160))){
                            
                            
                            h3fold->Fill(efficiency_eppip.M2());
                            MX2pim = efficiency_eppip.M2();
                            ppim = efficiency_eppip.P();
                            itheta = ( (efficiency_eppip.Theta()*TMath::RadToDeg()-theta_min_pim->Eval(ppim) )/ (theta_max_pim->Eval(ppim)-theta_min_pim->Eval(ppim)) );
                            if (mPi2-Exc_pi_window<MX2pim && MX2pim< mPi2+Exc_pi_window){
                                h2_pim_3fold_exc->Fill(ppim,itheta);
                            } else if (bgnd_low<MX2pim && MX2pim <bgnd_hi) {
                                h2_pim_3fold_bgnd->Fill(ppim,itheta);
                            }
                            
                            h_momP3fold->Fill(efficiency_eppip.P());
                            h_efficiency_eppipX->Fill(efficiency_eppip.M2());
                            h_3foldthetaVsp->Fill(efficiency_eppip.P(),efficiency_eppip.Theta()*TMath::RadToDeg());
                            
                            h_3foldthetaVsphi->Fill(efficiency_eppip.Phi()*TMath::RadToDeg(),efficiency_eppip.Theta()*TMath::RadToDeg());
                        ...
                        {
                            jndex = n_piminus[kPart];
                            
                            if(jndex==0) {
                                printf("piminus error n_piminus[%d] = %d \n", kPart, jndex);
                                continue;
                            }
                            px = particles[jndex]->par()->getPx();
                            py = particles[jndex]->par()->getPy();
                            ...
                            //Efficiency Studies for 4_fold
                            h_Missing_total_mass->Fill(Miss_total.M2());
                            h_Missing_total_energy->Fill(Miss_total.E());
                            // h1->Fill(Miss_pim.M2());
                            //fiducial cuts
                            double phiRot_4fold=Miss_pim.Phi()*TMath::RadToDeg() - (sin(Miss_pim.Theta())/Miss_pim.P())*70;
                            ...
                            if(((-20 <phiRot_4fold) && (phiRot_4fold < 20)) || ((40 <phiRot_4fold) && (phiRot_4fold < 80)) || ((100 <phiRot_4fold) && (phiRot_4fold < 140)) || ((-80 <phiRot_4fold) && (phiRot_4fold < -40)) || ((-140 <phiRot_4fold) && (phiRot_4fold < -100)) || ((160 <phiRot_4fold) || (phiRot_4fold < -160))){
                                ...
                                if(abs(Miss_total.E()) < 0.2 && abs(Miss_total.M2()) < 0.1){
                                    ...
                                    h2->Fill(Miss_pim.M2());
                                    h_momP4fold->Fill(Miss_pim.P());
                                    h_4foldtheta->Fill(Miss_pim.Theta()*TMath::RadToDeg());
                                    h_4foldphi->Fill(Miss_pim.Phi()*TMath::RadToDeg());
                                    h_4foldthetaVsphi->Fill(Miss_pim.Phi()*TMath::RadToDeg(),Miss_pim.Theta()*TMath::RadToDeg());
                                    h_4foldthetaVsp->Fill(Miss_pim.P(),Miss_pim.Theta()*TMath::RadToDeg());
                                    h_sinthetaPvsPhi->Fill(Miss_pim.Phi()*TMath::RadToDeg(),sin(Miss_pim.Theta())/Miss_pim.P());
                                    
                                    //(piplusFD)theta(pi+) vs p(pi+)and (piminusFD)theta(pi-)  vs p(pi-)
                                    h_4foldthetaVsP_piplus->Fill(piplus.P(),piplus.Theta()*TMath::RadToDeg());
                                ...
                                double d_theta, eff_val, theta_bin;
                                //    Make your 2D histogram of exclusive pim theta vs p.
                                h_4foldthetaVsP_piminus->Fill(piminus.P(),piminus.Theta()*TMath::RadToDeg());
                                ...
                                ppim_4fold = Miss_pim.P();
                                itheta_4fold = ( (Miss_pim.Theta()*TMath::RadToDeg()-theta_min_pim->Eval(ppim_4fold) )/ (theta_max_pim->Eval(ppim_4fold)-theta_min_pim->Eval(ppim_4fold)) );
                                if (mPi2-Exc_pi_window<MX2pim_4fold && MX2pim_4fold< mPi2+Exc_pi_window){
                                    h2_pim_4fold_exc->Fill(ppim_4fold,itheta_4fold);
                                } else if (bgnd_low<MX2pim_4fold && MX2pim_4fold <bgnd_hi) {
                                    h2_pim_4fold_bgnd->Fill(ppim_4fold,itheta_4fold);
                                    //    printf("ppim,itheta %f %f \n", ppim_4fold,itheta_4fold);
                                }
            ...
            }  //  Should all of these go inside the new "index" loops?
    ...
    TH1F *effMomP= (TH1F*)h_momP4fold->Clone("h_momP4fold");
    effMomP->SetTitle("(4fold pi- missing/3 fold pi- missing) as a function of momentum  ");
    effMomP->GetXaxis()->SetTitle(" P (GeV)");
    effMomP->GetYaxis()->SetTitle("Efficiency");
    effMomP->Divide(h_momP3fold);
    
    
    TCanvas *canefficiency  = new TCanvas();
    canefficiency->Divide(3,1);
    canefficiency->cd(1); h_momP3fold->DrawCopy();
    canefficiency->cd(2); h_momP4fold->DrawCopy();
    canefficiency->cd(3);effMomP->DrawCopy("E1");
    
    
    TH2F *effthetaVSP= (TH2F*)h_4foldthetaVsp->Clone("h_4foldthetaVsp");
    effthetaVSP->SetTitle("(4fold pi- missing/3 fold pi- missing)  ");
    effthetaVSP->GetXaxis()->SetTitle("");
    effthetaVSP->GetYaxis()->SetTitle("");
    effthetaVSP->Divide(h_3foldthetaVsp);
    
    
    TCanvas *canefficiency1  = new TCanvas();
    canefficiency1->Divide(3,1);
    canefficiency1->cd(1); h_3foldthetaVsp->DrawCopy("colz");
    canefficiency1->cd(2); h_4foldthetaVsp->DrawCopy("colz");
    canefficiency1->cd(3);effthetaVSP->DrawCopy("colz");
    ...
    int bmin3 = axis3->FindBin(-0.08);
    int bmax3 = axis3->FindBin(0.08);
    double Signal = h3fold->IntegralAndError(bmin3,bmax3,error3,"");
    TAxis *axis4 = h3norm.GetXaxis();
    int bmin4 = axis4->FindBin(-0.08);
    int bmax4 = axis4->FindBin(0.08);
    double  Bgnd = h3norm.IntegralAndError(bmin4,bmax4,error4,"");
    double Trues = Signal-Bgnd;
    TAxis *axis5 = h2->GetXaxis();
    int bmin5 = axis5->FindBin(-0.08);
    int bmax5 = axis5->FindBin(0.08);
    double  Coinc = h2->IntegralAndError(bmin5,bmax5,error5,"");
    double  Efficiency = Coinc/Trues;
    double errEff=(Coinc/Trues) *sqrt((1/Coinc)+(1/Trues));
    double myerrEff= (Coinc/Trues)* TMath::Sqrt((error5/Coinc)*(error5/Coinc)+(error3/Signal)*(error3/Signal)+(error4/Bgnd)*(error4/Bgnd));
    
    
    std::cout << "Efficiency = " << Efficiency << " +- " << myerrEff << std::endl;
    //  std::cout << "Efficiency = " << Efficiency  << std::endl;
    
    
    
    
    //Calculate error by hand for Trues
    /* error on Integral(h3norm,[b1,b2]) =bgdNorm * sqrt{Integral(h3,[b1,b2]) }
     =  sqrt{ bgdNorm * Integral(h3norm,[b1,b2]) }
     notice how the factor bgdNorm moves inside the sqrt.
     When you subtract two quantities, the errors ADD (in quadrature)
     Error on Trues = sqrt{ [Error on Signal]^2 + [Error on Bgd]^2 }
     = sqrt{ Signal + bgdNorm*Bgdn }*/
    
    /*  TAxis *axis6 = h3fold->GetXaxis();
     int bmin6 = axis6->FindBin(0.3);
     int bmax6 = axis6->FindBin(0.5);
     double norm3tot_hand = h3fold->Integral(bmin6,bmax6);
     TAxis *axis7 = h3->GetXaxis();
     int bmin7 = axis7->FindBin(0.3);
     int bmax7 = axis7->FindBin(0.5);
     double norm3bck_hand = h3->Integral(bmin7,bmax7);
     printf("norm3bck_hand %f\n",norm3bck_hand);
     double bgdNorm=norm3tot_hand/norm3bck_hand;
     printf("bgdNorm %f\n",bgdNorm);
     double errh3norm=bgdNorm*sqrt(h3->Integral(bmin7,bmax7));
     //h3norm
     TH1F h3norm_hand=  (norm3tot_hand/norm3bck_hand)*(*h3);
     TAxis *axis9 = h3norm_hand.GetXaxis();
     int bmin9 = axis9->FindBin(-0.1);
     int bmax9 = axis9->FindBin(0.1);
     double  Bgnd_hand = h3norm_hand.Integral(bmin9,bmax9);
     double errh3norm1=sqrt(bgdNorm*(h3norm_hand.Integral(bmin9,bmax9)));
     
     //Error on signal
     TAxis *axis8 = h3fold->GetXaxis();
     int bmin8 = axis8->FindBin(-0.1);
     int bmax8 = axis8->FindBin(0.1);
     double Signal_hand = h3fold->Integral(bmin8,bmax8);
     double Trues_hand= Signal_hand- Bgnd_hand;
     double errSignal=sqrt(h3fold->Integral(bmin8,bmax8));
     //Trues=Siganl-Backd
     double errTrues=sqrt(errSignal*errSignal + errh3norm*errh3norm);
     double errTrues1= sqrt(Signal_hand+Bgnd_hand*bgdNorm );
     std::cout << "errh3norm = " << errh3norm << " errh3norm1 " << errh3norm1 << std::endl;
     std::cout << "Trues_root = " << Trues <<  std::endl;
     std::cout << "Trues_hand = " << Trues_hand  << std::endl;
     TCanvas *caneffiicency3=new TCanvas();
     caneffiicency3->cd(1);
     h3fold->Draw();
     h3norm.Draw("sames");*/
    
    TCanvas *caneffiicency2=new TCanvas();
    h_4foldthetaVsP_piminus->DrawCopy("colz");
    theta_max_pim->DrawCopy("same");
    theta_min_pim->DrawCopy("same");
    
    TCanvas *caneffiicency23=new TCanvas();
    ...
    /*  TCanvas* ECinnerouter= new TCanvas();
     ...
     h_betapmag_CTOF_positive->DrawCopy("colz");
     CpmagDelta_CTOF->cd(2);
     h_betapmag_CTOF_negative->DrawCopy("colz");
     ...
     h_betapmag_CTOF_proton->DrawCopy("colz");
     CpmagDelta_CTOF_PID->cd(2);
     h_betapmag_CTOF_piplus->DrawCopy("colz");
     CpmagDelta_CTOF_PID->cd(3);
     h_betapmag_CTOF_piminus->DrawCopy("colz");
...
{
    /*
     Double_t thMax0 = par[0]*deg2rad(); // convert maximum theta from deg to rad
     Double_t  p1 = par[1]; // endpoint of region of constant theta_max
     Double_t alpha = par[2]; // coefficient alpha/p term.
     Double_t pMin = par[3]; // minimum momentum allowed
     */
    Double_t result = 0.0;
    if(par[1]==0.0) return result;
    Double_t  A0 = par[0] - par[2]/par[1]; // constant term for x[0] = p-> par[1]
    if (x[0]<par[3]){
        return result;
    } else if ( x[0]<par[1]){//------------Is this x[0]> par[1]? No x[0]<par[1]
        result = par[0];
    } else {
        result=A0 + par[2]/x[0];
    }
